...
module.exports = function(config) {
  ...
  // Add the middleware.
  router.formio.middleware = require('./src/lib/middleware/middleware')(router);
  ...
  router.init = function(hooks) {
    ...
    // Get the hook system.
    router.formio.hook = require('./src/lib/util/hook')(router.formio);

    // Get the encryption system.
    router.formio.encrypt = require('./src/lib/util/encrypt');

    // Load the updates and attach them to the router.
    router.formio.update = require('./src/lib/db/index')(router.formio);
    ...
    router.formio.update.initialize(function(err, db) {
      ...
      router.formio.auth = require('./src/lib/authentication/index')(router);
      ...
      mongoose.connection.once('open', function() {
        ...
        // Load the BaseModel.
        router.formio.BaseModel = require('./src/lib/models/BaseModel');

        // Load the plugins.
        router.formio.plugins = require('./src/lib/plugins/plugins');

        router.formio.schemas = {
          PermissionSchema: require('./src/lib/models/PermissionSchema')(router.formio),
          AccessSchema: require('./src/lib/models/AccessSchema')(router.formio)
        };

        // Get the models for our project.
        const models = require('./src/lib/models/models')(router);
        ...
        // Load the Resources.
        router.formio.resources = require('./src/lib/resources/resources')(router);

        // Load the request cache
        router.formio.cache = require('./src/lib/cache/cache')(router);
        ...
        router.formio.actions = require('./src/lib/actions/actions')(router);

        // Add submission data export capabilities.
        require('./src/lib/export/export')(router);

        // Add the available templates.
        router.formio.templates = {
          default: _.cloneDeep(require('./src/lib/templates/default.json')),
          empty: _.cloneDeep(require('./src/lib/templates/empty.json'))
        };

        // Add the template functions.
        router.formio.template = require('./src/lib/templates/index')(router);

        const swagger = require('./src/lib/util/swagger');
